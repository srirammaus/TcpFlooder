...
$con = mysqli_connect("localhost", "root", "password", "iinfo2") or  die("connection failed".mysqli_errno($con));
...
/*
//$key=1;
$profile_data= "SELECT * FROM iinfo2.student WHERE email= '$email';";
...
if($row==1){
   ...
   if($printer['password'] ==$pass and $printer['email'] ==$email){
      $flag=0;
   }else{
      $flag=-1;
   }

   //------$printer=mysqli_fetch_assoc($profile_data1); we no need this anymore --------
